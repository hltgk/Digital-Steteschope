...
rrUpdateInterval = fftUpdateInterval * 4; % RR güncelleme aralığı
delay = 0.00000001; % Döngü gecikmesi (saniye)
Fs = 2000; % Örnekleme frekansı (Hz)
maxFreq = 1000; % Maksimum frekans limiti (Hz)
...
figure('Name', 'Real-Time Stetescope Monitor with Filter (Lung)', 'Position', [1, 50, 1540, 735], 'UserData', struct('s', s, 'isRunning', false));

% Ham sinyal için grafik oluşturma
subplot(2, 2, 1);
h1 = plot(data, 'LineWidth', 1.5, 'Color', "#77AC30");
title('Real-Time Raw Signal (Lung)');
xlabel('Sample (n)');
ylabel('Signal');
grid on;
...
subplot(2, 2, 2);
h2 = plot(freqAxis, zeros(1, freqLimitIndex), 'LineWidth', 1.5, 'Color', "#77AC30");
title('Real-Time FFT of Raw Signal (Lung)');
xlabel('Frequency (Hz)');
ylabel('Magnitude');
grid on;
...
subplot(2, 2, 3);
h3 = plot(filteredData, 'LineWidth', 2, 'Color', "#7E2F8E");
title('Real-Time Filtered Signal (Lung)');
xlabel('Sample (n)');
ylabel('Signal');
grid on;
...
subplot(2, 2, 4);
h4 = plot(freqAxis, zeros(1, freqLimitIndex), 'LineWidth', 2,'Color', "#7E2F8E");
title('Real-Time FFT of Filtered Signal (Lung)');
xlabel('Frequency (Hz)');
ylabel('Magnitude');
grid on;
set(gca, 'Position', [0.54, 0.21, 0.44, 0.33]);

% Respiration Rate göstergesi
rrText = uicontrol('Style', 'text', 'Position', [30, 40, 50, 40], 'String', 'RR: -- breaths/min', 'FontSize', 12, 'FontWeight', 'bold');
...
sliderLow = uicontrol('Style', 'slider', 'BackgroundColor', [0.8, 0.8, 0.8], 'Min', 10, 'Max', maxFreq-10, 'Value', 180, 'Position', [880, 65, 580, 30], 'SliderStep', [10/(maxFreq - 10), (20/maxFreq)+0.000408], 'Tag', 'sliderLow', 'Callback', @(src, event) updateFilter());
sliderLowValue = uicontrol('Style', 'text', 'FontSize', 12, 'FontWeight', 'bold', 'Position', [1470, 70, 50, 20], 'String', '180', 'Tag', 'sliderLowValue');
...
sliderHigh = uicontrol('Style', 'slider', 'BackgroundColor', [0.8, 0.8, 0.8], 'Min', 10, 'Max', maxFreq-10, 'Value', 970, 'Position', [880, 20, 580, 30], 'SliderStep', [10/(maxFreq - 10), (20/maxFreq)+0.000408], 'Tag', 'sliderHigh', 'Callback', @(src, event) updateFilter());
sliderHighValue = uicontrol('Style', 'text', 'FontSize', 12, 'FontWeight', 'bold', 'Position', [1470, 25, 50, 20], 'String', '970', 'Tag', 'sliderHighValue');
...
counter = 0;
fftCounter = 0;
rrCounter = 0; % RR güncelleme sayacı
    ...
    if userData.isRunning
        if s.NumBytesAvailable >= 2
            ...
            if tempIndex > updateInterval
                ...
                rrCounter = rrCounter + updateInterval;
                ...
                if rrCounter >= rrUpdateInterval
                    rr = calculateRR(filteredData, Fs);
                    set(rrText, 'String', sprintf('RR: %.1f', rr));
                    rrCounter = 0;
                end
...
% Respiration Rate hesaplama
function rr = calculateRR(signal, Fs)
    % Sinyal genliğine dayalı eşik değeri belirle
    peakThreshold = mean(signal) + 0.5 * std(signal);
    [~, locs] = findpeaks(signal, 'MinPeakHeight', peakThreshold, 'MinPeakDistance', Fs * 1.5); % 1.5 saniye aralık
    
    if numel(locs) < 2
        rr = 0; % Yeterli tepe noktası yoksa RR hesaplanamaz
        return;
    end
    
    intervals = diff(locs) / Fs; % Tepe noktaları arasındaki süre (saniye cinsinden)
    avgInterval = mean(intervals); % Ortalama süre
    rr = 60 / avgInterval; % RR hesaplama (bir dakikadaki nefes sayısı)
end
